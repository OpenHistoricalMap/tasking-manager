...
export const setOSMTeamsDetails = (osmteams_token, osmteams_oauth_token, refresh_token) => (dispatch) => {
  const encoded_token = btoa(osmteams_token);
  safeStorage.setItem('osmteams_token', encoded_token);
  safeStorage.setItem('osmteams_refresh_token', refresh_token);
  safeStorage.setItem('osmteams_oauth_token', osmteams_oauth_token);
  dispatch(updateOSMTeamsToken(encoded_token));
};
